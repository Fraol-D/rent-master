...
const ImageInteractor2: React.FC<ImageInteractorProps> = ({ room }) => {
      ...
      {images.length > 0 ? (
        <>
          <div className="image-container" style={{ position: 'relative', width: '100%', height: '100%' }}>
            ...
            />
            <button className="arrow-button left" onClick={prevImage} style={{ position: 'absolute', left: 0, top: '50%' }}>
              <img src={RightArrow} alt="Previous" style={{ transform: 'rotate(180deg)' }} />
            </button>
            <button className="arrow-button right" onClick={nextImage} style={{ position: 'absolute', right: 0, top: '50%' }}>
              <img src={RightArrow} alt="Next" />
            </button>
            <div className="controls" style={{ position: 'absolute', bottom: 0, left: 0, right: 0, display: 'flex', justifyContent: 'space-between' }}>
              <button onClick={handleDeleteImage}>Del</button>
              <button onClick={() => handleShowInExplorer(images[currentIndex])}>Files</button>
