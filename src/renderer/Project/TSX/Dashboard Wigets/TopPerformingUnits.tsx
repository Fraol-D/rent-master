...
const TopPerformingUnits = ({
  ...
  const topTenants = useMemo(() => {
    return AllTenants.map((tenant) => {
      ...
      .slice(0, showAll ? AllTenants.length : 5);
  }, [AllTenants, RoomList, showAll]);
  ...
  const loyalTenants = useMemo(() => {
    return AllTenants.filter((tenant) => tenant.RentingOrOut)
      .map((tenant) => {
        const stayDuration =
  ...
  const { AllRoomPayInfo, setAllRoomPayInfo } = useGlobal();
  useEffect(() => {
  ...
  const topLifetimeRoomIncome = useMemo(() => {
    ...
    const roomIncomes = RoomPayInfo.reduce(
      (acc, payment) => {
        if (payment.Paid) {
          ...
          if (
            RoomList.find((r) => r.id === payment.roomId)?.Currency !==
            defaultCurrency
          ) {
            ...
            {activeTab === 'loyal' && (
              <TopList
                ...
                valueFormatter={(duration: number) =>
                  `${Math.floor(duration / (1000 * 60 * 60 * 24))} day` +
                  (Math.floor(duration / (1000 * 60 * 60 * 24)) > 1 ? 's' : '')
                }
              />
            )}
            {activeTab === 'lifetime' && (
    ...
    <div>
      <ul style={{ listStyleType: 'none', padding: 0 }}>
        {items.map((item: any, index: any) => (
          <li
            key={index}
            style={{ marginBottom: 'var(--5px-V)' }}
            title={`${index + 1}. ${item[
              valueKey
            ].toLocaleString()} ${CurrencySign(defaultCurrency)}`}
          >
            {index + 1}.{' '}
            <strong>
              Flr: {item.floor}, Rm: {item.roomIndex}
            </strong>
            : {formatNumberWithSuffix(item[valueKey])}{' '}
